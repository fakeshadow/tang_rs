...
//
//    let statements = vec![
//        ("SELECT * FROM topics WHERE id=ANY($1)", vec![Type::OID_ARRAY]),
//        ("SELECT * FROM users WHERE id=ANY($1)", vec![])
//    ];
//
//    // limitation for actix-web:
//    // since actix-web still use tokio 0.1 reactor under the hood so we can't spawn new postgres connections unless we replace the executor in src.
//    // so it's safe to set min_idle the same as max_size and idle_timeouot max_lifetime both to None.
...
//}
//
//fn test(pool: Data<Pool<tokio_postgres::NoTls>>) -> impl Future01<Item=HttpResponse, Error=Error> {
...
//                    .try_fold(
//                        (Vec::with_capacity(20), Vec::with_capacity(20)), |(mut t, mut u), r| {
//                            t.push(Topic {
//                                id: r.get(0),
//                                user_id: r.get(1),
//                                category_id: r.get(2),
